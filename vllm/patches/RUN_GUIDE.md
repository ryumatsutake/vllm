...
**语义 scope（Attention/MoE/Norm/RoPE 命名区间）**：加 `--scopes`，harness 通过 forward
hook 注入 `record_function`（和 RTP `at::RecordFunction` 一样是 Kineto user_annotation）。
注意 **scope 只在 `--enforce-eager` 下命中**（CUDA graph replay 不走 Python forward）；
且 scope 时长是 **CPU wall time**（eager 解码 CPU-bound 时它才是主要成本），GPU 耗时看
kernel 分类表。

```bash
python -u -m ...perf_test_runner ... --enforce-eager --scopes \
  --profile --profile-mode torch --profile-output /tmp/traces_eager --analyze
```
